...
全体の一括整形やライセンスヘッダの更新は、1 コミットで全モジュールの組に共起を作るが、知識は何も共有していない。
原著の結合は「変更が伝播する関係」なので、伝播していない変更は数えない。解析対象のファイルを `--max-commit-files`（既定 50）より多く触ったコミットは履歴ごと外す。
変動性と所有者の重なりからも同時に消える。`--max-commit-files=0` で無効にできる。

10 年分の履歴がある公開 OSS の PHP プロジェクトで測ると、3,358 件中 35 件が外れる（最大の一括整形コミットは PHP を 800 ファイル以上触っている）。同時変更率の平均は 0.048 から 0.023 へ半減し、
一緒に変わっているように見えていた 33 組は、共有するコミットが 1 件もなくなる。

この値を変えると測るものが変わるので、**ある値で書いた基準（`--baseline`）は別の値と比べられない**。変えたら `--write-baseline` で基準を作り直す。
=======
...
| `--max-commit-files` | 50 | 解析対象のファイルをこの数より多く触ったコミットを履歴から外す。一括整形やライセンス更新は、知識を共有しないまま全組に共起を作るため。`0` で無効 |
| `--weight-by-references` | なし | 順位づけを参照数の対数で重み付けする。原著は数ではなく性質を見る立場なので既定では使わない。参照 1 箇所の組が上位を埋めて読みにくいときに |
| `--format` | text | 出力の形（`text` / `json` / `samples` / `github`）。`--json` と `--samples` は同じものの別名 |
| `--fail-on` | なし | 均衡度がこの値以下の組があれば終了コード 1 を返す |
| `--baseline` | なし | 既知の組を記録したファイル。あれば、そこから増えた組と悪くなった組だけを見る |
| `--write-baseline` | なし | 今の計測で `--baseline` のファイルを書き直して終わる |

## CI で使う

結合は 1 度直して終わりではなく、増えるほうが速い。基準を 1 度置いて、そこから増えたぶんだけを止める使い方を用意してある。

```bash
# 1. いまの状態を基準として記録し、コミットする
vendor/bin/coupling-meter . --include=src --baseline=.coupling-baseline.json --write-baseline

# 2. CI では基準からの増分だけを見る。均衡度 3 以下の組が増えたら落とす
vendor/bin/coupling-meter . --include=src --baseline=.coupling-baseline.json --fail-on=3 --format=github
```

既存のコードベースに当てると崩れている組は最初から大量にあるので、全部を落とすと CI が永久に赤になる。基準を置けば、直す順は人が決めたまま、**新しく増えた崩れだけ**を止められる。
...
| コード | 意味 |
|---|---|
| 0 | `--fail-on` の閾値を超えた組は増えていない |
| 1 | 閾値以下の組が基準から増えた |
| 2 | 測定対象がない（モジュールが 1 つしかなく、組が 0 件）。名前空間がフラットなプロジェクトでは結合を測れないので、緑では通さない |
  ...
  balance:
    ...
    steps:
      - uses: actions/checkout@v5
        with:
          fetch-depth: 0   # 変動性と同時変更に git 履歴が要る
...
`fetch-depth: 0` を忘れると git 履歴が浅くなり、変動性と同時変更が出ない。
